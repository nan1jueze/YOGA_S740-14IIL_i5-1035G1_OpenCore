...
function dGR() {
  ...
  if [[ -n ${GITHUB_ACTIONS+x} || ${gh_api} == false ]]; then
    ...
    url=( "https://github.com$(curl -g -L --silent "${rawURL}" | grep '/download/' | grep -m 1 RELEASE | sed 's/^[^"]*"\([^"]*\)".*/\1/')" )
  else
    rawURL="https://api.github.com/repos/$1/releases$tag"
    url=( "$(curl -g --silent "${rawURL}" | grep 'browser_download_url' | grep -m 1 RELEASE | tr -d '"' | tr -d ' ' | sed -e 's/browser_download_url://')" )
  fi
